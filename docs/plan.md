...
- **Visible-constellation determination**: grid-sample the visible hemisphere. For altitude 0..90° and azimuth 0..360° in 2° steps, convert each horizontal point to a J2000 equatorial point and classify it:
  ...
  ```
  Verified live during planning: 2° resolution (8,280 points, ~15ms) found the same 46-constellation set as 0.5° (~68ms) *for the single London fixture tested*. **Correction (Phase 2 inspection, 2026-08-03):** that one-observer check did not generalize — a 162-observer sweep found a uniform 2° grid under-reports a near-horizon constellation in ~20% of cases vs 0.5°. The shipped code therefore uses a hybrid grid: 2° over the full hemisphere plus a dense 0.5° band over alt 0–5° (where all the misses live), which matches 0.5° in 0/162 cases at ~9.7ms. This is a grid-sampling approximation of the true boundary-polygon test that reuses astronomy-engine's own precise boundary data — it satisfies the spec's "precise IAU-88 boundary polygons" requirement without hand-rolling polygon-edge/horizon intersection math ourselves.
...
**Status:** [x] Complete — 2026-08-03. 429/500 friendly messages and no-coordinate-logging were already shipped in Phase 1 (verified, not rebuilt). Built this phase: explicit `message: "nothing bright visible right now"` field on `SkySnapshot` (API + chart empty-state rendering), `helmet`, `README.md`/`Procfile`. EARS walkthrough found one spec/code mismatch (Sky Calculation's Sun/Moon line had no corresponding output/code anywhere) — corrected the spec text rather than adding new scope; see `docs/decisions.md`. **Post-build security fix:** the first `trust proxy` implementation hardcoded `1`, which a live review caught as a rate-limit bypass on any deployment without a real reverse proxy in front (a spoofed `X-Forwarded-For` gets its own bucket per request). Fixed to default to `false` (key on the real socket connection) unless the `TRUST_PROXY` env var explicitly opts in — see `docs/learnings.md`.
